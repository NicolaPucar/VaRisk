#' This function calculates the VaR (Value at Risk) of a portfolio created by the user based on the asset-normal approach.
#' 
...
#' @export
#' @usage 
#' ValueAtRisk.AssetNormalApproach(confidenceLevel=0.95,timePeriod=1,daysForVariance=TRUE,stock1,weight1,stock2,weight2, ... )
#' @examples
#' GetReturns.YahooFinance("AAPL")
#' GetReturns.YahooFinance("MSFT")
#' GetReturns.YahooFinance("SNAP")
#' ValueAtRisk.AssetNormalApproach(,,,AAPL,3000,MSFT,5000,SNAP,2000)
#' ValueAtRisk.AssetNormalApproach(0.99,,,AAPL,3000,MSFT,5000,SNAP,2000)
#' ValueAtRisk.AssetNormalApproach(,10,,AAPL,3000,MSFT,5000,SNAP,2000)
#' ValueAtRisk.AssetNormalApproach(,,250,AAPL,3000,MSFT,5000,SNAP,2000)


ValueAtRisk.AssetNormalApproach <- function(confidenceLevel=0.95,timePeriod=1,daysForVariance=TRUE,stock1,weight1,
...
# Preparation of data ---------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  # - allNotUsedWeight: a vector that contains the part of the capital that we didn't use
  myPortfolio <- stock1
...
# Creation of the Var-Covar matrix ---------------------------------------------------------------------------------------------------------------------------------------------------
  varCovMatrix <- matrix(rep(0,numberOfFirms^2),nrow=numberOfFirms)
  if (daysForVariance==TRUE) {
    for (k in 1:numberOfFirms) {
      for (j in 1:numberOfFirms) {
        varCovMatrix[k,j] <- cov(myPortfolio[,k*2],myPortfolio[,j*2])
      }
    }
  } else {
    initialValue <- dim(myPortfolio)[1]-daysForVariance+1
    lastValue <- dim(myPortfolio)[1]
    for (k in 1:numberOfFirms) {
      for (j in 1:numberOfFirms) {
        varCovMatrix[k,j] <- cov(myPortfolio[initialValue:lastValue,k*2],myPortfolio[initialValue:lastValue,j*2])
      }
    }
  }
# Calculation of VaR ----------------------------------------------------------------------------------------------------------------------------------------------------
  # The following part of the code calculates the VaR based on an Asset-Normal Approach
  alpha <- qnorm(confidenceLevel)
  valueAtRiskResult <- alpha*as.double(sqrt(usedWeight%*%varCovMatrix%*%usedWeight))*sqrt(timePeriod)
# Results ---------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  print(paste0("The VaR(",confidenceLevel,";",timePeriod, ") based on an Asset-Normal Approach of your portfolio is:"))
